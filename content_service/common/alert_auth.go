...
func SendMail(addr string, auth smtp.Auth, from string, to []string, subject string, msg []byte) error {
  c, err := smtp.Dial(addr)
  host, _, _ := net.SplitHostPort(addr)
  if err != nil {
    logger.Errorf("call dial err: %v", err)
    return err
  }
  defer c.Close()
  ...
  if ok, _ := c.Extension("STARTTLS"); ok {
    ...
    if err = c.StartTLS(config); err != nil {
      logger.Errorf("call start tls, err: %v", err)
      return err
    }
  }
  ...
  if auth != nil {
    if ok, _ := c.Extension("AUTH"); ok {
      if err = c.Auth(auth); err != nil {
        logger.Errorf("check auth with err: %v", err)
        return err
      }
    }
  }
