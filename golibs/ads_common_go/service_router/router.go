...
import (
	...
	"github.com/algo-data-platform/predictor/golibs/ads_common_go"
	"github.com/algo-data-platform/predictor/golibs/ads_common_go/common/cityhash"
...
func getServerListHash(serverList ServerList) uint64 {
	...
	for idx := range serverList {
		...
		result = cityhash.CityHash64WithSeed(str, uint64(len(str)), result)
	}
	return result
}
...
func (router *Router) initVersionMetrics(scope tally.Scope) {
	scope.Tagged(map[string]string{"type": ROUTER_MAJOR_VERSION}).Gauge(ROUTER_VERSION).Update(ads_common_go.MAJOR_VERSION)
	scope.Tagged(map[string]string{"type": ROUTER_MINOR_VERSION}).Gauge(ROUTER_VERSION).Update(ads_common_go.MINOR_VERSION)
	scope.Tagged(map[string]string{"type": ROUTER_REVISION}).Gauge(ROUTER_VERSION).Update(ads_common_go.REVISION)
}
